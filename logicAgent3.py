...
input_path = './samples_v4/sample02.txt'
output_path = './log.txt'
...
def print_log(atomic, theta=None):
    ...
    for i, argument in enumerate(atomic.argument_list):
        if is_variable(argument) and type(theta) is not int and argument in theta:
...
def bw_or(kb, goal, theta):
    for i, rule in enumerate(kb.fetch_rules(goal, theta)):
        if type(rule) is int:
            yield 1
            return
...
def main():
    ...
    flag = last[0]
    end_writer = open(output_path, 'a')
    if flag is 'T':
        end_writer.write('True')
    else:
